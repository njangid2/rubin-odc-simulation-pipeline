...
def get_intensity_satellite_frame(
    ...
    for surface in sat_surfaces:
        ...
        #intensity = intensity + lumos.constants.SUN_INTENSITY * surface.area * (sun_contribution + earth_contribution) / np.mean(dist_sat_2_obs) ** 2
    return intensity

def get_diffuse_intensity_satellite_frame(
    sat_surfaces, 
    sat_height, 
    angle_past_terminator,
    observer_position,
    ...
    for surface in sat_surfaces:
        ...
        if include_sun:
            ...
            total_brdf = total_brdf + sun_contribution
            sun_contribution = sun_contribution + surface_brdf * surface_normalization * observer_normalization
...
def get_brdfs(
    ...
    for surface in sat_surfaces:
        ...
        if include_sun:
            ...
            print(surface_brdf)
            """
            sun_contribution = sun_contribution + surface_brdf 

        intensity = intensity + sun_contribution 
...
def get_diffuse_intensity_observer_frame(
    sat_surfaces, 
    sat_heights, 
    sat_altitudes, 
    sat_azimuths,
    ...
    for (i, (x, y, z, alpha, h)) in enumerate(zip(obs_x, obs_y, obs_z, angle_past_terminator, sat_heights)):
        intensity[i] = get_diffuse_intensity_satellite_frame(
            sat_surfaces, h, alpha, (x, y, z),
            include_sun, include_earthshine, earth_panel_density, earth_brdf, diffuse_brdf)
